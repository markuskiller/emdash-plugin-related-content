...
const CAPABILITIES = ["content:read", "hooks.page-fragments:register"];
export function relatedContentPlugin(options) {
    return {
        id: PLUGIN_ID,
...
export function createPlugin(options = { relationships: [] }) {
    return definePlugin({
        id: PLUGIN_ID,
        version: PLUGIN_VERSION,
        capabilities: [...CAPABILITIES],
        hooks: {
            "page:fragments": async (event, ctx) => {
                ...
                if (page.kind !== "content" || !page.content?.collection || !page.content.id)
                    return null;
                if (!ctx.content)
                    return null;
                const relationships = options.relationships.filter((relationship) => relationship.sourceCollection === page.content?.collection && relationship.renderMode !== "manual");
                ...
                if (!sourceEntry)
                    return null;
                const sections = await getRelatedContent({ sourceEntry: sourceEntry, relationships, content });
